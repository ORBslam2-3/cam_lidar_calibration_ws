...
namespace extrinsic_calibration
{
    ...
    void feature_extraction::onInit()
    {
        ...
        std::cout<<" ready entry extractROI??? "<<std::endl;
                                                   ...
                                                   pcl::PointCloud<pcl::PointXYZIR>::Ptr &right_up)
    {
        // Visualize minimum and maximum points
        visualization_msgs::Marker minmax;
        minmax.header.frame_id = "laser_link";          
        minmax.header.stamp = ros::Time();
        minmax.ns = "my_sphere";
        minmax.type = visualization_msgs::Marker::SPHERE;
        minmax.action = visualization_msgs::Marker::ADD;
    ...
    {
        std::cout<<" extractROI begin extract feature "<<std::endl;
        ...
        pass1.setFilterLimits(bound.x_min, bound.x_max);  //bound.x_min  bound.x_max 参数值在哪设置? 是为了滤除点云，手动设置的
        ...
        if (flag == 1)
        {
            ...
            std::cout<<"flag=1 begin extract feature"<<std::endl;
            ...
            marker1.header.frame_id = line_strip.header.frame_id = corners_board.header.frame_id = "laser_link";
