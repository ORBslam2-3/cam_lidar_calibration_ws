...
void Projection::image_pointcolud_cb(const sensor_msgs::Image::ConstPtr &ori_image, const sensor_msgs::PointCloud2::ConstPtr &ori_pointcloud)
{
   ...
   //----------------------------------------------------------------------- 
    ...
    cv::undistort(cv_image_src->image, image_undistorted, camera_matrix, distortion_coefficients);

    cv_bridge::CvImage cv_image_reprojected;
    cv_image_reprojected.header = cv_image_src->header;
    cv_image_reprojected.encoding = cv_image_src->encoding;
    cv_image_reprojected.image = image_undistorted.clone();
    ...
    for (int i = 0; i < cloudSize; i++)
    {
        ...
        if (planepointsC.y >= 0 and planepointsC.y < image_height and planepointsC.x >= 0 and planepointsC.x < image_width and
            tmpzC >= 0 and std::abs(tmpxC) <= 2.35) //1.35
        {
            ...
            cv::circle(cv_image_reprojected.image,
                       cv::Point(planepointsC.x, planepointsC.y), point_size,
                       CV_RGB(255 * colmap[50 - range][0], 255 * colmap[50 - range][1], 255 * colmap[50 - range][2]), -1);
        }
    }
    ...
    reproject_image_pub_.publish(cv_image_reprojected.toImageMsg());
}
